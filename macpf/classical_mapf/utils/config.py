...
The full parameter list lives in configs/default.yaml (single source of
truth). This module only loads it and exposes it as an object.
"""
...
# The registry of every simulation parameter. Required file.
# This file is macpf/macpf/classical_mapf/utils/config.py, so the project root
# (which holds configs/) is four levels up.
...
class Config:
    ...
    config.max_speed_mps == values["max_speed_mps"]. Accessing a key that
    does not exist in configs/default.yaml raises immediately.
    """
...
def load_config(path: Optional[str] = None) -> Config:
    ...
    The overlay may only use keys that exist in default.yaml; unknown keys
    raise an error so typos do not silently do nothing.
    """
    import yaml

    if not DEFAULT_CONFIG_PATH.exists():
        raise FileNotFoundError(
            f"Required config registry not found: {DEFAULT_CONFIG_PATH}"
        )
...
# Backward-compatible alias: type annotations across modules say MAPFConfig.
MAPFConfig = Config
