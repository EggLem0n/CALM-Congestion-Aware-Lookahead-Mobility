...
def main() -> None:
    ...
    # True-distance A* heuristic. On the obstacle-heavy factory map Manhattan is a
    # weak lower bound, so each A* fans out across large areas before threading
    # around the conveyors -- the dominant re-plan cost. A per-goal BFS distance
    # field is the exact obstacle-aware heuristic; cached once per goal and reused
    # across every agent and re-plan. Off -> Manhattan (exact original routes).
    heuristic_provider = None
    if bool(config.online_astar_true_distance_heuristic):
        _distance_field_cache: dict = {}

        def heuristic_provider(goal):  # noqa: E306  -- goal -> cached BFS distance field
            key = (int(goal[0]), int(goal[1]))
            field = _distance_field_cache.get(key)
            if field is None:
                field = compute_distance_field(walkable_map, key)
                _distance_field_cache[key] = field
            return field

        print("A* heuristic: true BFS grid-distance (obstacle-aware, cached per goal).")
