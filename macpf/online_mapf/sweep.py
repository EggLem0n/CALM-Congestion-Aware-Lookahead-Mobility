...
def run_job(planner, seed, config, max_time, num_agents, device, out_file,
            video=False, subframes=None, skip_fraction=None, replan_every=None) -> Dict:
    cmd = [sys.executable, "-m", PLANNER_MODULE[planner], "--config", config, "--seed", str(seed)]
    ...
    if planner == "online":
        cmd += ["--device", device]
        # online-only knobs; classical has no such flags so we never forward them there.
        if skip_fraction is not None:
            cmd += ["--congestion-skip-fraction", str(skip_fraction)]
        if replan_every is not None:
            cmd += ["--replan-every", str(replan_every)]
    if video:
        # Full run with a per-seed run-id so parallel jobs don't collide, animation forced on.
        run_id = f"s{seed}"
        cmd += ["--run-id", run_id, "--save-animation"]
        if subframes is not None:
            cmd += ["--subframes", str(subframes)]
        metrics_path = PROJ_ROOT / "data" / f"{planner}_runs" / run_id / "classical_metrics.json"
    else:
        cmd += ["--metrics-out", str(out_file)]
        metrics_path = Path(out_file)
...
def to_markdown(agg, seeds, n_ok, n_total, stamp, online_note="") -> str:
    arrow = {"up": "↑", "down": "↓", None: "—"}
    lines = [
    ...
    if online_note:
        lines.append(f"- online 옵션: {online_note}")
    lines += [
        "",
...
def main() -> None:
    ...
    parser.add_argument("--congestion-skip-fraction", type=float, default=None,
                        help="online 전용: A* 혼잡도 항을 무시하는 AMR 비율(0..1, 예 0.3). config 값을 덮어씀.")
    parser.add_argument("--replan-every", type=int, default=None,
                        help="online 전용: 몇 스텝(초)마다 재계획할지(예 5). config 값을 덮어씀.")
    parser.add_argument("--video", action="store_true",
                        help="Also render a per-seed MP4 (full run per seed; slower) instead of metrics-only.")
    parser.add_argument("--subframes", type=int, default=3,
                        help="Video speed when --video: 3=10x, 30=real-time (at 30 fps).")
    ...
    # online-only override summary (recorded in the report for reproducibility).
    opt_bits = []
    if args.congestion_skip_fraction is not None:
        opt_bits.append(f"skip_fraction={args.congestion_skip_fraction}")
    if args.replan_every is not None:
        opt_bits.append(f"replan_every={args.replan_every}")
    online_note = ", ".join(opt_bits)

    jobs = [(p, s) for s in seeds for p in planners]
    print(f"sweep: {len(jobs)} jobs ({planners} × {len(seeds)} seeds), "
          f"{args.num_of_process} processes, config={args.config}"
          + (f", online[{online_note}]" if online_note else ""), flush=True)
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.num_of_process)) as ex:
        futs = {
            ex.submit(run_job, p, s, args.config, args.max_time, args.num_agents,
                      args.device, runs_dir / f"{p}_seed{s}.json", args.video, args.subframes,
                      skip_fraction=args.congestion_skip_fraction,
                      replan_every=args.replan_every): (p, s)
            for (p, s) in jobs
        }
    ...
    md = to_markdown(agg, seeds, n_ok, len(jobs), stamp, online_note)
    ...
    (out_dir / "sweep.json").write_text(json.dumps({
        ...
        "online_congestion_skip_fraction": args.congestion_skip_fraction,
        "online_replan_every": args.replan_every,
