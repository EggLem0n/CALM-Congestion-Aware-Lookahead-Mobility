...
from macpf.classical_mapf import (
    ...
    has_continuous_motion_conflict_indexed,
    has_edge_conflict,
    has_vertex_conflict,
)
...
def replan(
    ...
    agent_ai_cost_weights: Optional[Sequence[float]] = None,
    wait_diagnostics: Optional[Dict[str, int]] = None,
) -> List[PathType]:
    ...
    for agent_id in priority_order:
        ...
        if agent_id not in skip_agents and agent_ai_cost_weights is not None:
            agent_config = planning_config.replace(
                use_ai_congestion_cost=True,
                ai_cost_weight=float(agent_ai_cost_weights[agent_id]),
            )
        ...
        if not segment or len(segment) <= 1:
            ...
            used_safe_extension = True
            segment, _ = extend_path_safely(
                [pos], reservation_table, edge_reservation_table, walkable_map, planning_config
            )

        if wait_diagnostics is not None and segment and len(segment) > 1 and segment[1] == pos:
            wait_diagnostics["planned_wait_total"] = wait_diagnostics.get("planned_wait_total", 0) + 1
            if used_safe_extension:
                wait_diagnostics["wait_due_to_safe_extension"] = (
                    wait_diagnostics.get("wait_due_to_safe_extension", 0) + 1
                )
            else:
                classify_first_step_wait(
                    pos,
                    agent.goal,
                    walkable_map,
                    reservation_table,
...
def classify_first_step_wait(
    ...
    edge_reservation_table: Dict[int, set],
    congestion_cost: np.ndarray,
    agent_config: MAPFConfig,
    config_congestion_off: MAPFConfig,
    soft_cost_table: Dict[int, Dict[Coord, float]],
    edge_buckets: Dict,
    wait_diagnostics: Dict[str, int],
) -> None:
    """Record the likely reason an A* result waits on its first step.

    This is a diagnostic, not another planner. If every moving neighbour is
    blocked by hard constraints, the wait is constraint-driven. If at least one
    moving step is legal, compare against a no-AI-cost A* plan: when disabling
    the AI term makes the first step move, the wait was caused by congestion
    cost. Otherwise it is likely soft proximity / ordinary path-cost tradeoff.
    """
    x, y = pos
    moving_candidates = [
        (nx, ny, nt) for nx, ny, nt in get_neighbors(x, y, 0, walkable_map)
        if (nx, ny) != pos
    ]
    ...
    for nx, ny, nt in moving_candidates:
        cell = (nx, ny)
        if has_vertex_conflict(nx, ny, nt, reservation_table):
            blocked_counts["wait_due_to_vertex_reservation"] += 1
            continue
        if has_clearance_conflict(nx, ny, nt, reservation_table, int(agent_config.hard_clearance_cells)):
            blocked_counts["wait_due_to_hard_clearance"] += 1
            continue
        if has_edge_conflict(pos, cell, nt, edge_reservation_table):
            blocked_counts["wait_due_to_edge_conflict"] += 1
            continue
        if has_continuous_motion_conflict_indexed(
            pos,
            cell,
            nt,
            edge_buckets,
            agent_config,
            float(agent_config.continuous_safe_gap_cells),
        ):
            blocked_counts["wait_due_to_continuous_conflict"] += 1
            continue
        legal_moves.append(cell)

    if not legal_moves:
        wait_diagnostics["wait_due_to_no_legal_move"] = (
            wait_diagnostics.get("wait_due_to_no_legal_move", 0) + 1
        )
        for key, value in blocked_counts.items():
            wait_diagnostics[key] = wait_diagnostics.get(key, 0) + int(value)
        return

    wait_diagnostics["wait_with_legal_move_available"] = (
        wait_diagnostics.get("wait_with_legal_move_available", 0) + 1
    )
    if goal is not None and bool(agent_config.use_ai_congestion_cost):
        no_ai_segment = astar_single_agent(
            pos,
            goal,
            walkable_map,
            reservation_table,
        ...
        if no_ai_segment is not None and len(no_ai_segment) > 1 and no_ai_segment[1] != pos:
            wait_diagnostics["wait_due_to_ai_congestion_cost"] = (
                wait_diagnostics.get("wait_due_to_ai_congestion_cost", 0) + 1
            )
            return

    wait_diagnostics["wait_due_to_soft_or_path_cost"] = (
        wait_diagnostics.get("wait_due_to_soft_or_path_cost", 0) + 1
    )
