...
class TaskManager:
    ...
    def __init__(
        ...
        initial = [self._sample_from_groups(self.pickup_groups) for _ in range(num_agents)]
    ...
    def _sample_from_groups(self, groups: Dict[str, List[Coord]]) -> Tuple[str, Coord]:
        all_points = [(name, point) for name, points in groups.items() for point in points]
        idx = int(self.rng.integers(0, len(all_points)))
    ...
    def _select_from_groups(
        ...
        *,
        ...
        if self.target_selector is not None:
            selected = self.target_selector(agent_id, tuple(position), action, groups)
            if selected is not None:
                return selected
        return self._sample_from_groups(groups)

    def _sample_pickup(self) -> Coord:
        zone, point = self._sample_from_groups(self.pickup_groups)
        self._last_zone = zone
        return point

    def _sample_delivery(self) -> Coord:
        zone, point = self._sample_from_groups(self.delivery_groups)
        self._last_zone = zone
        return point
    ...
    def update_arrivals(self, positions: Sequence[Coord], t: int) -> None:
        for agent_id, pos in enumerate(positions):
            ...
            if self.goal_is_pickup[agent_id]:
                ...
                zone, goal = self._select_from_groups(
...
def main() -> None:
    ...
    dispatch_rng = np.random.default_rng(int(config.seed) + 7301)

    def ai_target_selector(
        agent_id: int,
        position: Coord,
        action: str,
        groups: Dict[str, List[Coord]],
    ) -> Optional[Tuple[str, Coord]]:
        if args.mode != "ai" or not bool(args.ai_dispatch):
            return None
        del agent_id, action
        candidates = [(name, point) for name, points in groups.items() for point in points]
        if not candidates:
            return None
        scored: List[Tuple[float, str, Coord]] = []
        cost_index = min(1, current_cost.shape[0] - 1)
        for zone, point in candidates:
            x, y = point
            dist = distance_cache.distance(point, tuple(position))
            congestion = float(current_cost[cost_index, y, x]) if current_cost.size else 0.0
            score = float(args.dispatch_distance_weight) * dist + float(args.dispatch_congestion_weight) * congestion
            scored.append((score, zone, point))
        scored.sort(key=lambda item: item[0])
        top_k = max(1, min(int(args.dispatch_top_k), len(scored)))
        _, zone, point = scored[int(dispatch_rng.integers(0, top_k))]
        return zone, point
    ...
    task_manager = TaskManager(
        ...
        target_selector=ai_target_selector,
    )
    ...
    def candidate_cost_fn(agent_id: int, cell: Coord, goal: Coord, timestep: int) -> float:
        ...
        return float(ai_weights[agent_id]) * task_multiplier * raw_cost
    ...
    metrics["pibt_ai_dispatch_enabled"] = bool(args.ai_dispatch and args.mode == "ai")
    metrics["pibt_dispatch_distance_weight"] = float(args.dispatch_distance_weight)
    metrics["pibt_dispatch_congestion_weight"] = float(args.dispatch_congestion_weight)
    metrics["pibt_dispatch_top_k"] = int(args.dispatch_top_k)
