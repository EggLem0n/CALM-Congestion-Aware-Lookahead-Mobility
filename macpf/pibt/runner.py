    ...
    def update_arrivals(self, positions: Sequence[Coord], t: int) -> None:
        for agent_id, pos in enumerate(positions):
            ...
            if self.goal_is_pickup[agent_id]:
                ...
                goal = self._sample_delivery()
                self.goals[agent_id] = goal
                self.current_zone[agent_id] = self._last_zone
...
def main() -> None:
    ...
    def ai_priority_bias() -> List[float]:
        if args.mode != "ai" or float(args.ai_priority_weight) <= 0.0:
            return [0.0 for _ in positions]
        return [
            float(args.ai_priority_weight) * float(ai_weights[agent_id]) * normalized_ai_cost(pos)
            for agent_id, pos in enumerate(positions)
        ]
